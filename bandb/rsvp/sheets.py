...
class RsvpSheet(Client):
    ...
    _data_id_col = 'A'
    _data_code_col = 'B'
    _data_last_name_col = 'C'
    _data_last_col = 'I'
    _data_header_rows = 2
    
    _response_sheet = 'RSVP'
    _response_first_col = 'A'
    _response_id_col = 'A'
    _response_last_col = 'E'
    _response_header_rows = 2
...
#             return None
    ...
    def _col_index(self, col):
        return [c for c in col_range(self._data_first_col, self._data_last_col)].index(col)
...
#             return None
    ...
    def _get_invitee_by_rsvp_id(self, rsvp_id):
        return self._format_data(self._get_values(self._data_sheet, row(rsvp_id + self._data_header_rows)), self._data_sheet)[0]
...
#         try:
            ...
            if (normalize(last_name) not in normalize(data[self._headers[self._data_sheet][self._col_index(self._data_last_name_col)]])):
                return None
            return data
#         except:
#             return None
